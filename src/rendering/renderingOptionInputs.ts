...
import {
  attachTooltip,
  clampNumber,
  formatRenderingOptionTitle,
  getRenderingOptionStep,
  type RenderingOptionDefinition,
  type RenderingOptionValue
} from './renderingSettingsTypes';

export function createLinkedNumberSlider(
  parent: HTMLElement,
  options: {
    key: string;
    label: string;
...
export function createRenderingOptionInput(
  parent: HTMLElement,
  key: string,
  ...
  if (definition.type === 'bool') {
    ...
    return createNumberInput(item, inputId, key, definition, currentValue, resetValue, onChange, optionTitle);
  }
